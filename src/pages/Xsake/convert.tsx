...
const TabConvert: FC<ConvertProps> = ({
    sakes = 0
}) => {
    ...
    const openInput = async () => {
        ...
        if (parseFloat(inputValue) > sakes) {
    ...
    return (
        <Fragment>
            <h3>Get xSAKE</h3>
            <p>
                Unlock bonus rewards and exclusive benefits by converting your SAKE to xSAKE.
            </p>
            <Column style={{ width: "100%", padding: '10px 20px' }}>
                ...
                <Row style={{ marginTop: "10px", marginBottom: "15px", justifyContent: "flex-end" }}>
                    ...
                    <Text fontSize={14} style={{ paddingRight: "10px", cursor: 'pointer' }}> Balance:  &nbsp;
                        <span onClick={() => setInputValue(amountPretty((SuiUtils.d(sakes)), 8))}>
                            {amountPretty((SuiUtils.d(sakes)), 8)}
                        </span>
                    </Text>
                </Row>
                {error ?
                ...
                <RowBetween width="100%" style={{ padding: '10px 0px' }}>
                    ...
                    >
                        <Trans>
                            {account ? "Approve" : "Connect Wallet"}</Trans>
                    </ButtonPrimary>
